...
public class LushCaveType extends BasicCaveType
{
	...
	public void genFloor(WorldGenLevel world, INoise noise, BlockPos pos, float depth, int pass, RandomSource rand)
	{
		...
		if(pass == 0)
		{
			...
			if(this.supportsDeepslateDecor(world, pos) && -0.2d < d && d < 0.4d)
				this.replaceBlock(world, pos, Blocks.MOSS_BLOCK.defaultBlockState());
			else if(-0.2d < d && d < 0.4d)
				this.replaceBlock(world, pos, Blocks.DIRT.defaultBlockState());
			if(this.supportsDeepslateDecor(world, pos) && this.getNoise(noise, pos, 0.25d) < 0.2d)
				this.replaceBlock(world, pos, Blocks.TUFF.defaultBlockState());
			else if(this.getNoise(noise, pos, 0.25d) < 0.2d)
	...
	public void genFloorExtra(WorldGenLevel world, INoise noise, BlockPos pos, float depth, int pass, RandomSource rand)
	{
		...
		if(pass == 1)
		{
			...
			if(this.supportsDeepslateDecor(world, pos) && d < -0.4d)
	...
	public void genCeil(WorldGenLevel world, INoise noise, BlockPos pos, float depth, int pass, RandomSource rand)
	{
		...
		if(pass == 0)
		{
			if(this.supportsDeepslateDecor(world, pos) && this.getNoise(noise, pos, 0.125d) < -0.4d)
				this.replaceBlock(world, pos, Blocks.TUFF.defaultBlockState());
			else if(this.getNoise(noise, pos, 0.125d) < -0.4d)
				this.replaceBlock(world, pos, Blocks.DIRT.defaultBlockState());
			if(this.supportsDeepslateDecor(world, pos) && this.getNoise(noise, pos, 0.25d) < 0.1d)
				this.replaceBlock(world, pos, Blocks.MOSS_BLOCK.defaultBlockState());
			else if(this.getNoise(noise, pos, 0.25d) < 0.1d)
				this.modifyBlock(world, pos, SubWildLookups.MOSSY);
		}
		super.genCeil(world, noise, pos, depth, pass, rand);
	}
