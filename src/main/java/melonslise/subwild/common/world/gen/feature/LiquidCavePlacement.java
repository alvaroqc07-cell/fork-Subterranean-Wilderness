...
public class LiquidCavePlacement extends CavePlacement
{
	...
	public Stream<BlockPos> getPositions(PlacementContext context, RandomSource rand, BlockPos pos)
	{
		...
		if(chunk instanceof ProtoChunk protoChunk)
		{
			...
			if(!SubWildConfig.EXPENSIVE_SCAN.get())
				return mask.stream(chPos).filter(carvedPos -> this.isValidGenerationPos(level, chunk, structureManager, carvedPos));
		...
		for(int x = 0; x < 16; ++x)
			for(int z = 0; z < 16; ++z)
				for(int y = yMin, yMax = chunk.getHeight(Heightmap.Types.OCEAN_FLOOR, x, z); y < yMax; ++y)
				{
					...
					if(chunk.getBlockState(mut).getBlock() == Blocks.WATER && this.shouldDecorateWaterPos(rand) && this.isValidGenerationPos(level, chunk, structureManager, mut))
	...
	protected boolean shouldDecorateWaterPos(RandomSource rand)
	{
		return SubWildConfig.GENERATE_FEATURES_IN_WATER.get() || rand.nextInt(4) == 0;
	}
